<?php
    ...
    if (!empty($_POST["user-name"]) && !empty($_POST["email"]) && !empty($_POST["pass"])) {
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            $query = "INSERT INTO users (name, email, pass, role, img) VALUES ('$user_name', '$user_email', '$user_pass', '$user_role', '$user_img')";
            if (mysqli_query($link, $query)) {
                echo "Регистрация прошла успешно!";
                echo '<a href="../../index.php">На главную</a>';
            } else {
                echo "Процесс регистрации завершился ошибкой" . mysqli_error($link);
            }
        }
    }
    mysqli_close($link);
